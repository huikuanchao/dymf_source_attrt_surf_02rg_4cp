...
void read_gro( FILE *ip ) {
  ...
  int tp_ind2,tp_ind1 = nD*(Nda+Ndb) + nA*Nha +nB*Nhb + nP * ( 1 + ng_per_partic * ( Ng + 1 ) ) + max_nC*Nhc ;

  tp_ind2 = tp_ind1 +(nC - max_nC)*Nhc +nsol;
...
void write_film_gro(){
  FILE *otp ;
  int tp_ind2,tp_ind1,i, j, ind, k, m, resind ;
  if ( step == 0 ) 
    otp = fopen( "film_sol.gro" , "w" ) ;
  else
    otp = fopen( "film_sol.gro" , "a" ) ;
 ...
 for ( k=0 ; k<max_nsol ; k++ ) {
     if(sol_flag[k] >0){
      ...
      fprintf( otp , "%5d" , (tp_ind1+tmp_solind) % 100000 + 1 ) ;
