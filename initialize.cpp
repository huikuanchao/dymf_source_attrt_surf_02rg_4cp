...
void initialize() {
  int tp_ind3 ,tp_ind1, tp_ind2,i,j ;

  idum =   -long( time(0) ) ; // 9 ; //

//  read_input() ;

  if ( phiP + phiHA + phiHB + phiHC+phisol> 1.0 )
  ...
  chiAs = chiAs / double( Nda + Ndb ) ;
  chiBs = chiBs / double( Nda + Ndb ) ;
  chiCs = chiCs / double( Nda + Ndb ) ;
 ...
 double  rdc_V = V * ( wall_para > 0 ? (L[2]-wall_thick*2)/L[2]: 1.0 );
  ...
  nC = int( phiHC * rho0 * rdc_V / Nhc * CG_ratio +0.5) ; 
  nsol = int( phisol * rho0 * rdc_V /CG_ratio +0.5) ;
  max_nC = int( 0.5 +(phisol+ phiHA + phiHB +phiHC)*rho0 *rdc_V/ Nhc *CG_ratio) ;
  max_nsol = max_nC*Nhc ;  //int(  * rho0 * rdc_V /CG_ratio ) ;
  Vp = rho0 ;

  if( nsol > max_nsol or nC > max_nC) {
	cout<<"wrong nsol or nC set up"<<endl;
	exit(1);
  }
  ...
  rho0 = ( nsol+ nD * (Nda + Ndb ) + nA * Nha + nB * Nhb + nC*Nhc+nP * (Vp + ng_per_partic * Ng ) ) / rdc_V / CG_ratio ;
  printf("actual rho0: %lf\n" , rho0 ) ;

  printf("\nnD: %d\nnA: %d\nnB: %d\nnC: %d\nnP: %d\nnsol: %d\n" , nD, nA, nB, nC, nP, nsol ) ;

  // Derived quantities //
  nstot =  nA * Nha + nB * Nhb + nD * ( Nda + Ndb ) +  max_nsol +max_nC*Nhc  + nP * ( 1 + ng_per_partic * ( Ng + 1 ) ) ;
  ...
  I = complex<double>( 0.0 , 1.0 ) ;
    ...
    printf("Total segments ( max nc and sol ): %d\n" , nstot ) ;
    printf("grid_vol: %lf\n" , gvol ) ;
  ...
  tp_ind1 = nD*(Nda+Ndb) + nA*Nha +nB*Nhb + nP * ( 1 + ng_per_partic * ( Ng + 1 ) ) ;
  tp_ind2 = tp_ind1 + max_nC*Nhc;
  tp_ind3 = tp_ind2+max_nsol;

  for(j = 0; j<nstot ;j++){

	if(j<tp_ind1)
        	nstot_flag[j] = 1 ; 
        else if(j >=tp_ind1 and (j<(tp_ind1+nC*Nhc)))
	       nstot_flag[j] = 1 ;
	else if( j>= tp_ind2 and (j<(tp_ind2+nsol) ) )
  	       nstot_flag[j] =1 ;
  	else
	       nstot_flag[i] =0;
  }
  ...
  printf("lined-lists initialized, written\n") ; fflush( stdout ) ;
}
...
sizeof(complex<double>*));
   ...
   //ncgn_bac = ( double** ) calloc( eq_nsol , sizeof( double* ) ) ;
   //solgn_bac = ( double** ) calloc( eq_nsol , sizeof( double* ) ) ;
   gn_bac = ( double** ) calloc( nstot , sizeof( double* ) ) ;
   x_bac = ( double** ) calloc( nstot , sizeof( double* ) ) ;
   //xsol_bac = ( double** ) calloc( max_nsol , sizeof( double* ) ) ;

  //xsol =  ( double** ) calloc( max_nsol , sizeof( double* ) ) ;
  //xnc_bac = ( double** ) calloc( max_nc , sizeof( double* ) ) ;

  //xnc =  ( double** ) calloc( mac_nc , sizeof( double* ) ) ;


  x = ( double** ) calloc( nstot , sizeof( double* ) ) ;
  xc = ( char** ) calloc( nstot , sizeof( char* ) ) ;
  f = ( double** ) calloc( nstot , sizeof( double* ) ) ;
 // fnc = ( double** ) calloc(  max_nc , sizeof( double* ) ) ;
 // fsol = ( double** ) calloc( max_sol , sizeof( double* ) ) ;
  sol_flag = ( int* ) calloc( max_nsol , sizeof( int ) ) ;
  nc_flag = ( int* ) calloc( max_nC , sizeof( int ) ) ;
  nstot_flag = ( int* ) calloc( nstot , sizeof( int ) ) ;
 ...
 /* for ( i=0 ; i<max_sol ; i++ ) {
   //  xsol_bac[i] = ( double* ) calloc( Dim , sizeof( double ) ) ;
   //  solgn_bac[i] = ( double* ) calloc( Dim , sizeof( double ) ) ;
   //  xsol[i] = ( double* ) calloc( Dim , sizeof( double ) ) ;
   //  fsol[i] = ( double* ) calloc( Dim , sizeof( double ) ) ;
  }


  for ( i=0 ; i<max_nc ; i++ ) {
	xnc_bac[i] = ( double* ) calloc( Dim , sizeof( double ) ) ;
	ncgn_bac[i] = ( double* ) calloc( Dim , sizeof( double ) ) ;
	xnc[i] = ( double* ) calloc( Dim , sizeof( double ) ) ;
        fnc[i] = ( double* ) calloc( Dim , sizeof( double ) ) ;
  }
 ...
 mem_use += nP * (ng_per_partic+ 3*27+3*2)* sizeof( double ) ;
  ...
  rhosol = ( double* ) calloc( M , sizeof( double ) ) ;
  ...
  rhosol_t = ( double** ) calloc( nthreads , sizeof( double* ) ) ;
  ...
  for ( i=0 ; i<nthreads ; i++ ) {
    ...
    rhosol_t[i] = ( double* ) calloc( M , sizeof( double ) ) ;
  ...
  for ( j=0 ; j<Dim ; j++ ) {
    ...
    gradwsol[j] = ( double* ) calloc( M , sizeof( double ) ) ;
