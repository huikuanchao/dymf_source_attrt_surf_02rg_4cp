...
#pragma omp parallel for
  for ( i=0 ; i<M ; i++ ) {
       rhosol[i] = rhohc[i] = rhoda[i] = rhoha[i] = rhodb[i] = rhohb[i] = rhop[i] = smrhop[i] = rhoga[i] = 0.0 ;
    for ( j=0 ; j<nthreads ; j++ ) 
      rhosol_t[j][i]=rhohc_t[j][i] = rhoda_t[j][i] = rhoha_t[j][i] = rhodb_t[j][i] = rhohb_t[j][i]  = rhoga_t[j][i] = rhop_t[j][i] = 0.0 ;
  }
...
#pragma omp parallel for private(j)
  for ( i=0 ; i<M ; i++ ) {
    for ( j=0 ; j<nthreads ; j++ ) {
      ...
      rhohb[i] += rhohb_t[j][i] ;
      rhohc[i] += rhohc_t[j][i] ;
      rhosol[i] += rhosol_t[j][i] ;
      rhop[i] += rhop_t[j][i] ;
      rhoga[i] += rhoga_t[j][i] ;
    }

    rhot[i] = rhohc[i]+rhoda[i] + rhosol[i] + rhodb[i] + rhoha[i] + rhohb[i]  + rhoga[i];
  ...
  if ( Dim == 3 ) {
    for ( ix = 0 ; ix < pmeorder+1 ; ix++ ) {
      ...
      for ( iy = 0 ; iy < pmeorder+1 ; iy++ ) {
	    ...
	    rhosol_t[tid][ Mindex ] += W3 / CG_ratio ;
	  else {
            char nm[40] ;
